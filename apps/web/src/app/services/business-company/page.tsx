...
import { Settings, ArrowLeft, ArrowRight, TrendingUp, ShieldCheck, ChevronRight } from "lucide-react";
...
export default function BusinessCompanyPage() {
  const subroutes = [
    {
      ...
      href: "/services/business-company/predictive-analytics-dashboards",
      desc: "Synthesize operational and customer metrics. We create high-performance interactive portals modeling historical trends and future forecast curves.",
      icon: <TrendingUp className="text-[#F59E0B]" size={20} />,
      badge: "BI & Modeling"
    },
    {
      name: "Regulatory Compliance RAG",
      href: "/services/business-company/regulatory-compliance-rag",
      desc: "Enforce strict governance rules. Grounded semantic architectures check documentation, policies, and transactions against active legal directives.",
      icon: <ShieldCheck className="text-[#10B891]" size={20} />,
      badge: "Grounded AI Agent"
    }
  ];
  ...
  const jsonLdSchema = {
    ...
    "@type": "CollectionPage",
    "@id": "https://bagpackers.dev/services/business-company#collection",
    "name": "Business Intelligence & Compliance Services",
    "description": "Predictive analytics, interactive metric dashboards, and grounded compliance checker tools.",
    "publisher": {
      "@type": "Organization",
    ...
    <>
      ...
      <main className="min-h-screen bg-[#FAFAFA] text-zinc-600 antialiased selection:bg-[#10B891]/30 selection:text-zinc-900 pb-20 overflow-x-hidden">
        ...
        <nav className="max-w-6xl mx-auto px-6 pt-8 text-xs font-bold text-zinc-500 flex items-center gap-1.5 uppercase tracking-wider">
          ...
          <span>/</span>
          <span className="text-zinc-900">Business Solutions</span>
        </nav>
        ...
        <section className="max-w-4xl mx-auto px-6 pt-16 pb-20 text-center space-y-6">
          ...
          >
            ...
            Operational Intelligence Vertical
          </motion.div>
          ...
          >
            Business Analytics & <span className="text-gradient">Legal Governance</span>.
          </motion.h1>
          ...
          >
            Empower your team with evidence-led execution. We deploy predictive dashboards modeling operation metrics, paired with deterministic semantic compliance checks to reduce risks.
          </motion.p>
        </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {subroutes.map((route, idx) => (
              <motion.div
                ...
                whileHover={{ 
                  y: -5,
                  borderColor: idx === 0 ? "#F59E0B" : "#10B891",
                  boxShadow: `0 10px 25px -5px ${idx === 0 ? "rgba(245, 158, 11, 0.15)" : "rgba(16, 184, 145, 0.15)"}`
                }}
